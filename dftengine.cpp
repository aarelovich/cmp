...
void DFTEngine::run(){
    running = false;
    qWarning() << "Starting DFT";
    ComplexVectorPointer dft = calcDFT(buffer);
    qWarning() << "DFT has ended";
    dftResults << dft;
    running = true;
}

ComplexVectorPointer DFTEngine::getDFTResult(){
    ComplexVectorPointer dft;
    if (dftResults.isEmpty()) return dft;
    dft = dftResults.first();
    dftResults.removeFirst();
    return dft;
}
